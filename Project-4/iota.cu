...
int main(int argc, char* argv[]) {
    ...
    int chunkSize = 256;
    Count numChunks = (numValues + chunkSize - 1) / chunkSize;
    iota<<<static_cast<unsigned int>(numChunks), chunkSize>>>(numValues, gpuValues, startValue);
  
    cudaMemcpy(values.data(), gpuValues, numBytes, cudaMemcpyDeviceToHost);

    Count step = numValues / NumCheckValues;
    for (Count i = 6, n = 0; i < numValues && n < NumCheckValues; ++n, i += step) {
